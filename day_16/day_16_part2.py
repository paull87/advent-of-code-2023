...
def simulate_beam(layout, beam_layout, start_pos, start_dir):
    beams = [
        {'pos': start_pos, 'direction': start_dir},
    ]
    while beams:
        del_beams = []
        new_beams = []
...
def run_all_simulations(layout):
    results = {}
    edges = []
    edges += [((x, 0), 'd') for x in range(len(layout[0]))]
    edges += [((x, len(layout)-1), 'u') for x in range(len(layout[0]))]
    edges += [((0, y), 'r') for y in range(len(layout))]
    edges += [((len(layout[0])-1, y), 'l') for y in range(len(layout))]
    print(edges)
    for start_pos, start_dir in edges:
        beam_layout = create_beam_map(layout)
        simulate_beam(layout, beam_layout, start_pos, start_dir)
        beam_path = beams_paths(beam_layout)
        results[start_pos + (start_dir, )] = beam_power(beam_path)

        print(start_pos, start_dir, beam_power(beam_path))
...
def main():
    ...
    all_sims = run_all_simulations(layout)
    pprint(all_sims)
    return max(all_sims.values())

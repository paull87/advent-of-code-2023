...
def score_hand(hand):
    ...
    j_less_hand = hand.replace('J', '')
    jokers = len(hand) - len(j_less_hand)
    ...
    # cater for jokers and override score
    if jokers > 0:
        if jokers == 4:
            score = CARD_ORDERS['5OK']
        if jokers == 3:
            if len(unique_hand) == 2:
                score = CARD_ORDERS['5OK']
            elif len(unique_hand) == 3:
                score = CARD_ORDERS['4OK']
        if jokers == 2:
            if score == CARD_ORDERS['FH']:
                score = CARD_ORDERS['5OK']
            elif score == CARD_ORDERS['3OK']:
                pass
            elif score == CARD_ORDERS['2P']:
                score = CARD_ORDERS['4OK']
            elif score == CARD_ORDERS['1P']:
                score = CARD_ORDERS['3OK']
        if jokers == 1:
            if score == CARD_ORDERS['4OK']:
                score = CARD_ORDERS['5OK']
            elif score == CARD_ORDERS['3OK']:
                score = CARD_ORDERS['4OK']
            elif score == CARD_ORDERS['2P']:
